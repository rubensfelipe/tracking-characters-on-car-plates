...
ajuste_solar = imsubtract(g1,50); %fotos com sol na placa
ajuste_sol = ajuste_solar>60; 
...
for n = 1:size(Iprops_sol,1)
    box_sol = Iprops_sol(n).BoundingBox; %box_sol = [origem_em_x origem_em_y largura altura]
...
for n = 1:size(Iprops,1)
    box = Iprops(n).BoundingBox; %box = [origem_em_x origem_em_y largura altura]
...
%Matriz sem repetição das propriedades geométricas das boxs quem tenham pontos medianos
%próximos
...
%Identificando as propriedades geométricas de cada box (xbox3)
%Enquadrando cada box que esteja dentro dos intervalos desejados
xbox3 = [0 0 0 0];
xb3 = [0 0 0 0]; 
if size(xbox,1)>=7 || size(xbox2,1)>=7
    h = 0; u = 1;
